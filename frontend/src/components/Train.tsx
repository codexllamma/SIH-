...
interface TrainProps {
  ...
  debug?: boolean;
  route?: string[]; // sequence of track ids
  tracksData?: TrackDef[];
  loop?: boolean; // NEW: whether to loop or stop at the end
}
...
const Train: React.FC<TrainProps> = ({
  ...
  const buildPolyline = () => {
    if (!route || route.length === 0 || !tracksData || tracksData.length === 0) {
      polyRef.current = [{ x, y }, { x: x + 1000, y }];
  ...
  function detectRedSignalAhead(): number | null {
    ...
    for (const s of signals) {
      if (assignedSignals && !assignedSignals.map(String).includes(String(s.id)))
        continue;
        ...
        if (dRemaining > 0) {
          ...
          if (segIndexRef.current >= poly.length - 1) {
            if (loop) {
              ...
              // stop
              posRef.current = poly[poly.length - 1];
              setTick((t) => t + 1);
              return;
            }
          }
