...
class RailEnv(gym.Env):
    ...
    def step(self, action) -> Tuple[np.ndarray, float, bool, bool, dict]:
      """Enhanced step function with graduated collision handling and improved performance."""
      ...
      # Initialize collision position tracking if not exists
      if not hasattr(self, '_collision_positions'):
          self._collision_positions = {}
      if not hasattr(self, '_collision_cooldowns'):
          self._collision_cooldowns = {}

      # Clean old collision positions (performance optimization)
      if self.current_step % 100 == 0:  # Every 100 steps
          self._collision_positions = {k: v for k, v in self._collision_positions.items() if v < 5}
      ...
      # Enhanced proximity checks & collision detection with graduated penalties
      proximity_events = 0
      ...
      train_length = float(self.train_length)
      ...
      for t in range(self.n_trains):
          ...
          for u in range(t + 1, self.n_trains):
              ...
              # Identify trailing and leading trains
              if self.positions[t] < self.positions[u]:
                  trailing, leading = t, u
              else:
                  trailing, leading = u, t
                  
              trailing_speed = int(self.speeds[trailing])
              leading_speed = int(self.speeds[leading])
              braking_dist = float(c["braking_distance_map"].get(trailing_speed, 0))
              
              # GRADUATED COLLISION SYSTEM
              if distance < train_length:
                  # COLLISION: Handle more intelligently
                  collision_events += 1
                  self._episode_collision_count += 1
                  track_idx = int(self.tracks[t])
                  
                  # Position-based collision tracking for repeated offenses
                  pos_key = f"{int(self.positions[t]//10)}_{int(self.positions[u]//10)}"  # Discretize positions
                  self._collision_positions[pos_key] = self._collision_positions.get(pos_key, 0) + 1
                  
                  # Emergency braking instead of immediate disable
                  self.speeds[t] = 0
                  self.speeds[u] = 0
                  emergency_braking_events += 1
                  
                  # Shorter track blocking to allow recovery learning
                  self.track_blocked_timer[track_idx] = max(2, c.get("collision_block_recovery", 5) // 3)
                  
                  # Only disable after 3+ collisions in same area
                  if self._collision_positions[pos_key] >= 3:
                      self.disabled[t] = True
                      self.disabled[u] = True
                      
              elif distance < train_length * 1.8:
                  # CRITICAL PROXIMITY: Very dangerous
                  critical_proximity_events += 1
                  
              elif distance < train_length * 2.5:
                  # NEAR MISS: Close call but manageable
                  near_miss_events += 1
                  
              elif distance <= braking_dist:
                  # UNSAFE FOLLOWING: Within braking distance
                  proximity_events += 1
                  
              elif distance <= braking_dist * 1.3 and trailing_speed > leading_speed + 2:
                  # APPROACH WARNING: Fast train approaching slower one
                  proximity_events += 1

      # Movement delta calculation
      delta_pos = np.sum(np.maximum(0.0, self.positions - prev_positions))

      # Lateness calculation for active trains
      lateness_total = 0
      idle_count = 0
      for i in range(self.n_trains):
      ...
      # Build enhanced reward components
      comps = {
          ...
          "critical_proximity_events": int(critical_proximity_events),  # NEW
          "near_miss_events": int(near_miss_events),  # NEW
          "emergency_braking_events": int(emergency_braking_events),  # NEW
      ...
      # Compute reward using enhanced components
    ...
    def _compute_reward(self, comps: Dict[str, float]):
      """
      Enhanced reward computation with graduated collision penalties.
      ...
      # NEW graduated penalty weights
      w_critical_proximity = float(rc.get("w_critical_proximity", 0.35))
      w_near_miss = float(rc.get("w_near_miss", 0.15))
      w_emergency_braking = float(rc.get("w_emergency_braking", 0.10))
      ...
      # Annealing factor for penalties
      ...
      # GRADUATED PROXIMITY PENALTIES (much more stable than binary collision)
      weighted["proximity"] = -anneal * w_proximity * comps["proximity_events"]
      weighted["near_miss"] = -anneal * w_near_miss * comps["near_miss_events"]
      weighted["critical_proximity"] = -anneal * w_critical_proximity * comps["critical_proximity_events"]
      weighted["emergency_braking"] = -anneal * w_emergency_braking * comps["emergency_braking_events"]
      
      # Collision penalty (scaled but much smaller than before)
      collision_base_penalty = float(rc.get("collision_base_penalty", 100.0))  # Much smaller
      weighted["collision"] = -anneal * w_collision * comps["collision_events"] * collision_base_penalty / 100.0
